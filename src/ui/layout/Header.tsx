      ...
      >
        {HeaderItems.map((HeaderItem, index) => (
          <Link
            key={index}
            href={HeaderItem.url}
            className={`${pathName === HeaderItem.url ? "bg-white text-[#01322090]" : "bg-transparent text-white"} text-sm sm:text-base  hover:scale-105 transition-all uppercase font-semibold w-1/5 text-center rounded-lg p-1 sm:p-2`}
          >
            {HeaderItem.title}
          </Link>
        ))}
